...
class Huawei(ConsumerSQL):
    '''
    Class for taking data from RabbitMQ Huawei queue
    and adding it to MySQL Huawei table
    ...
    callback(ch, method, proprietes, body) -> None:
        Add RabbitMQ message to MySQL Huawei table
    '''
    def __init__(self):
        '''Start RabbitMQ and MySQL connections'''
        super().__init__()

        result = self.channel.queue_declare(**config.huawei_sql_queue)
        self.queue = result.method.queue
        self.channel.queue_bind(**config.huawei_sql_bind)

        self.table_prefix = config.huawei_table
