...
taking data from RabbitMQ Alcatel queue
and adding it to the Mongo Alcatel collection for that month
'''
import signal
import hashlib
import config
...
class Alcatel(Consumer_Mongo):
    '''
    Consumer Mongo for Alcatel vendor
    ...
    def __init__(self):
        ...
        self.collection_prefix = config.alcatel_collection

        # create queue
        result = self.channel.queue_declare(**config.alcatel_mongo_queue)
        self.queue = result.method.queue
        self.channel.queue_bind(**config.alcatel_mongo_bind)
    ...
    def gen_md5(self, line):
        ...
        '''
        unique_string = ""
        for field in config.md5_alcatel:
